...
uint8_t bitInx = 7;
	...
	float hum_reading = dhtData[0];
	hum_reading *= 0x100;					// >> 8
	hum_reading += dhtData[1];
	hum_reading /= 10;						// get the decimal

	// == get temp from Data[2] and Data[3]
	
	float temp_reading = dhtData[2] & 0x7F;	
	temp_reading *= 0x100;				// >> 8
	temp_reading += dhtData[3];
	temp_reading /= 10;
